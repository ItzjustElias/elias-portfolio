...
const TechLabel = ({
  ...
  <div
    className={`absolute font-mono text-[10px] tracking-[0.3em] text-zinc-600 uppercase ${className}`}
  >
    {label}
  </div>
);
...
const FloatingTech = ({
  ...
  <motion.div
    ...
    className="absolute font-mono text-[10px] text-white/90 border border-white/90 
           px-2 py-1 rounded-md bg-[#8A2BE2]/10 backdrop-blur-md 
           shadow-[0_0_10px_rgba(138,43,226,0.3)] z-20"
    style={{ left: x, top: y }}
  >
    {label}
  </motion.div>
    ...
    >
      <div className="fixed inset-0 pointer-events-none z-50">
        ...
        <TechLabel
          ...
          className="rotate-90 origin-left left-4 top-1/4"
        />
        <TechLabel
          label="Design.Unit: 01"
          className="-rotate-90 origin-right right-4 top-1/4"
        />
      </div>
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-12">
          {[
            ...
            <div key={i} className="flex flex-col gap-2">
              <span className="font-mono text-[10px] text-[#5D3FD3]">
                {stat.label}
              </span>
      ...
      <section ref={bentoRef} className="relative z-20 px-6 py-40 bg-black">
        <div className="max-w-7xl mx-auto relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 auto-rows-[300px]">
            <Link
              href="/project/zero"
              className="bento-card-anim md:col-span-2 md:row-span-2 group block h-full"
            >
              <motion.div
                whileHover={{ scale: 0.985 }}
                className="h-full relative bg-zinc-900/30 border border-white/10 rounded-[2.5rem] overflow-hidden p-12 flex flex-col justify-between hover:border-[#5D3FD3]/50 transition-all duration-500"
              >
                ...
                <div className="flex justify-between items-start z-10">
                  <div className="px-4 py-1.5 bg-[#5D3FD3]/10 border border-[#5D3FD3]/20 text-[#5D3FD3] rounded-full text-[10px] font-bold tracking-widest uppercase">
                    Selected Work
                  </div>
                  <div className="w-12 h-12 bg-white/5 rounded-full flex items-center justify-center group-hover:bg-[#5D3FD3] transition-all duration-500">
                    →
                  </div>
                </div>
                <div className="z-10">
                  <h3 className="text-6xl font-black uppercase tracking-tighter mb-4 italic">
                    ZERO
                  </h3>
                  <p className="text-zinc-400 text-sm max-w-62.5 mb-6">
                    Pushing the boundaries of digital interaction through 3D
                    experiences.
                  </p>
                </div>
              </motion.div>
            ...
            {["Motion", "Strategy", "Design", "Code"].map((item, i) => (
              <div
                key={i}
                className="bento-card-anim relative bg-zinc-900/30 border border-white/10 rounded-4xl p-8 flex flex-col justify-between group hover:bg-[#5D3FD3]/5 transition-all overflow-hidden"
              >
                <div className="flex justify-between items-center">
                  <span className="text-[#5D3FD3] font-mono text-xs">
                    0{i + 1}
                  </span>
                  <div className="w-2 h-2 rounded-full bg-zinc-800 group-hover:bg-[#5D3FD3] transition-colors" />
                </div>
                <h3 className="text-2xl font-bold uppercase tracking-tight group-hover:translate-x-2 transition-transform">
                  {item}
                </h3>
              </div>
            ))}
            ...
            {TECH_DATA.map((tech, i) => (
              ...
              >
                ...
                <div className="text-center">
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-widest group-hover:text-[#5D3FD3] transition-colors">
                    {tech.name}
                  </p>
                </div>
      ...
      <section className="relative z-30 py-35 bg-black px-6">
        <div className="max-w-7xl mx-auto">
          <h2 className="font-mono text-[10px] text-zinc-600 uppercase tracking-[0.5em] mb-20">
            Capabilities
          </h2>
        ...
        <div className="animate-marquee flex gap-12 whitespace-nowrap">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="flex gap-12 items-center">
              <span className="text-[10vw] font-black uppercase italic opacity-10 hover:opacity-100 hover:text-[#5D3FD3] transition-all duration-500">
                Innovation
              </span>
              <span className="text-[10vw] font-black uppercase italic opacity-10">
                •
              </span>
            </div>
          ))}
      ...
      <footer className="py-40 text-center relative z-30 bg-black">
        ...
        <a
          href="/privacy"
          className="hover:text-white transition-colors text-[10px] font-mono text-zinc-500 tracking-widest"
        >
          Privacy Policy
        </a>
      </footer>
